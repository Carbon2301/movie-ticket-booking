...
const paymentMethods = [
  { value: "CASH", label: "Cash at counter", icon: <Coins size={20} />, description: "Pay directly at the cinema counter", fee: "Free" },
  { value: "CREDIT_CARD", label: "Credit/Debit Card", icon: <CreditCard size={20} />, description: "Secure online payment", fee: "Free" },
  { value: "BANK_TRANSFER", label: "Bank Transfer", icon: <Banknote size={20} />, description: "Transfer directly from your bank", fee: "Free" },
  { value: "E_WALLET", label: "E-Wallet", icon: <Wallet size={20} />, description: "MoMo, ZaloPay, VNPay", fee: "Free" },
];
...
const PaymentPage = () => {
  ...
  const { seatCodes, totalPrice, ticketIds, movie, room, showTime } = location.state || {};
  ...
  const handlePay = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await paymentAPI.create({ ticketIds, method: paymentMethod });
      ...
      navigate("/payment-success", {
        state: { ...response.data, movie, room, showTime, seatCodes, totalPrice }
      });
    } catch (err) {
    ...
    <div className="relative flex flex-col md:flex-row min-h-screen items-center justify-center py-8 mt-20 px-4 md:px-0 background-color gap-4 md:gap-0">
      ...
      <div className="w-full max-w-sm bg-primary/8 border border-primary/20 rounded-2xl shadow-xl p-4 md:p-6 md:mr-8 flex flex-col gap-3 backdrop-blur-lg">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-extrabold text-white drop-shadow">🎫 Booking Details</h2>
          {timeLeft && (
        ...
        <div className="flex items-center gap-4">
          <img src={movie?.posterUrl} alt={movie?.title} className="w-24 h-36 object-cover rounded-xl shadow-lg border-2 border-primary/40" />
          <div>
            <div className="font-bold text-xl mb-1">{movie?.title || "Movie Title"}</div>
            <div className="text-base text-[#bbb9e6]">{room?.name}</div>
            <div className="text-sm text-gray-300 mt-2">
              <span className="text-gray-400">Showtime:</span> <b>{dateFormat(showTime)}</b>
            </div>
            <div className="text-sm text-gray-300 mt-1">
              <span className="text-gray-400">Seats:</span> <b>{seatCodes?.join(", ")}</b>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-2 text-sm text-[#e2deff]">
          <div className="flex items-center gap-2">
            ...
            <span><span className="text-gray-400">Cinema:</span> <span className="font-semibold">{room?.cinema?.name}</span></span>
          </div>
          <div className="flex items-start gap-2">
            <MapPin size={16} className="text-gray-400 mt-0.5" />
            <span><span className="text-gray-400">Address:</span> <span className="font-semibold text-white">{room?.cinema?.location}</span></span>
          </div>
          <div className="flex items-center gap-2">
            <Shield size={16} className="text-green-400" />
            <span className="text-green-300 text-xs">Your booking is protected</span>
          </div>
        </div>
        <div className="mt-4 flex items-center justify-between">
          <span className="text-xl text-white font-bold">Total Amount</span>
          <span className="text-3xl font-extrabold drop-shadow-glow">{totalPrice?.toLocaleString()}₫</span>
        </div>
      </div>
      ...
      <div className="w-full max-w-sm bg-primary/8 border border-primary/20 rounded-2xl shadow-xl p-4 md:p-6 flex flex-col gap-3 backdrop-blur-lg">
        <h2 className="text-2xl font-extrabold text-white drop-shadow">💳 Payment</h2>
        <label className="block text-base font-semibold mb-2 text-white">Select your payment method:</label>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="flex items-center gap-2">{pm.icon} {pm.label}</span>
                  <span className="ml-auto text-green-300 text-sm">{pm.fee}</span>
                </div>
                <div className="text-sm text-gray-300 ml-8">{pm.description}</div>
              </label>
            );
          })}
        </div>
